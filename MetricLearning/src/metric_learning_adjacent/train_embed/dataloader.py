...
class Hit_Pair_Dataset(Dataset):
    def __init__(self, data_filepath, nb_samples):
        ...
        try:
            self.hits_a = np.array(dataset['hits_a'][:nb_samples],dtype=np.float32)
            self.hits_b = np.array(dataset['hits_b'][:nb_samples],dtype=np.float32)
            self.target = np.array(dataset['target'][:nb_samples],dtype=np.float32)
        except:
